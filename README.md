...
[ComfyUI CCP API Exporter](https://github.com/potechitakusan/ComfyUI_CCP_API_Exporter) は、ComfyUIPlugin 用の API JSON を出力するための ComfyUI フロントエンド拡張です。ワークフロー画面上の値を変更せずに、キャンバス・SubImage・Prompt・Negative Prompt・num1〜num3・出力保存先を選択して、ComfyUIPlugin 用のマーカーへ置換した API JSON をダウンロードできます。

`ComfyUI/custom_nodes/ComfyUI_CCP_API_Exporter/` に Exporter のフォルダを配置して ComfyUI を再起動した後、対象ワークフローのキャンバスを右クリックし、`Export API JSON for CCP` を選択してください。`Output Save Image` は必ず選択し、必要な項目を指定して `Export` をクリックします。ダウンロードした JSON を ComfyUIPlugin のフォルダに配置して利用してください。

以下は Exporter を使わずに手作業で API JSON を作成する場合の手順です。
...
### Windows画像変換が失敗した場合（Pythonによる画像変換を利用）

通常、画像のPNG/BMP変換はC++からWindows標準のWICを利用して実行するため、Pythonの設定は不要です。

`debuglog.txt`に次のようなメッセージが出た場合、Windows標準の画像変換に失敗しています。

    IMAGE_CONVERSION_WIC_FAILED: PNG to BMP (...)

または

    IMAGE_CONVERSION_WIC_FAILED: BMP to PNG (...)

この場合は、Python（Pillow）による画像変換へ切り替えてください。

1. ComfyUIPluginフォルダの`UserSetting.ini`を開き、`[COMMON]`セクションへ以下を追加します。

       use_python_image_conversion = "true"

2. ComfyUIPluginフォルダの`png_to_bmp.bat`と`bmp_to_png.bat`を開き、先頭のPATHをPython環境に合わせて設定します。

       set path=C:\【ComfyUIのインストールフォルダ】\python_embeded;%path%

   StabilityMatrixの場合は、例えば以下のように設定します。

       set path=C:\【StabilityMatrixのインストールフォルダ】\Packages\ComfyUI\venv\Scripts;%path%

3. CLIP STUDIO PAINTを再起動し、再度フィルターを実行します。

Python画像変換を利用している場合に`debuglog_py.txt`へ`python.exe が存在しません`などのエラーが出た場合は、上記のPATH設定と、Python環境にPillowが入っていることを確認してください。

Windows標準の変換へ戻す場合は、`UserSetting.ini`の`use_python_image_conversion`を`false`にするか、該当行を削除します。
...
まずComfyUIPluginフォルダの「debuglog.txt」を確認してください。Python画像変換を有効にしている場合は、「debuglog_py.txt」も確認してください。

・`debuglog.txt`に`IMAGE_CONVERSION_WIC_FAILED`が出ます。

Windows標準の画像変換に失敗しています。READMEの「Windows画像変換が失敗した場合（Pythonによる画像変換を利用）」を参照し、`UserSetting.ini`でPythonを有効にしてから、batファイルのPython PATHを設定してください。

・Python画像変換で`python.exe が存在しません`のようなエラーが出ます。

「Windows画像変換が失敗した場合（Python画像変換を利用）」に記載したPATH設定を確認してください。Pythonを使わない既定のC++変換へ戻す場合は、`UserSetting.ini`の`use_python_image_conversion`を`false`にするか、該当行を削除します。
