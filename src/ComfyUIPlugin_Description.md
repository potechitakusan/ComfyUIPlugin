...
- `ComvertImage.cpp / .h`：Windows 標準の WIC を直接呼び出す画像形式変換。PNG/BMPを24bit BGRへ統一する。
- `bmp_to_png.py` / `png_to_bmp.py`：設定で有効化した場合のみ使う、従来のPython（Pillow）による画像変換。
...
2. 選択範囲を `temp_img_req.bmp` として保存し、`ComvertImage.cpp`（WIC）で PNG へ変換。
...
7. `/view` API で生成画像を `temp_img_res.png` として保存し、`ComvertImage.cpp`（WIC）でアルファなし24bit BMPに変換。
...
- `use_python_image_conversion`：`true`の場合のみ、従来のbat/Python（Pillow）で画像変換する。既定は`false`（WIC）。
...
- Pythonによる画像変換を利用する場合は、`.bat`内でPython環境のPATH設定が必要。既定のWIC変換では`.bat`を実行しない。

## 8. 外部依存と前提条件
- Windows 環境（Win32 API、`MultiByteToWideChar`、`fopen_s`、`GetModuleFileNameA` を使用）。
- `curl` コマンドが PATH で利用可能であること。
- 既定の画像変換はWindows標準のWICを使用する。Pythonによる画像変換を利用する場合のみ、Pillow (`PIL`) が組み込まれたPython実行環境が必要。
...
- WIC変換失敗時はHRESULTを`debuglog.txt`へ出力する。Pythonによる画像変換を利用している場合は、周辺スクリプト・バッチも確認する。

## 10. 制限事項・注意点
- 文字列処理は Shift-JIS（ANSI）→ UTF-16/UTF-8 変換を多用。テンプレート JSON やプロンプトは Shift-JIS で扱われる点に留意。
- Pythonによる画像変換を利用する場合、`.bat`内のパスをユーザー環境に合わせる必要がある。

## 11. リリース前チェック
1. `ComfyUIPlugin.ini` の `COMMON` セクションと各プリセットが最新仕様と一致しているか確認。
2. `SubImage` フォルダに想定の PNG サンプルが配置されているか確認。
3. WICによるBMP→PNG→BMP変換を検証する。Pythonによる画像変換を提供する場合はPython／Pillowも検証する。
...
本仕様書は `ComfyUIPlugin.cpp` とその関連ファイル群（`ComfyUIPlugin.h`、`FilterPlugIn.*`、WIC画像変換、Pythonによる画像変換スクリプト、`ComfyUIPlugin.ini`）を対象とし、`SDPlugin.cpp` 系統は範囲外である。
